...
def two_hours_close(h1, h2, time):
    ...
    if (max(h1, h2) - min(h1, h2)) < time:
        return True
    if (min(h1, h2) + Util.num_min_in_day) - max(h1, h2) < time:
        return True
    return False
...
def dist_between_hours(h1, h2):
    ...
    dist2 = (min(h1, h2) + Util.num_min_in_day) - max(h1, h2)
...
def recommend_sleep_time(wake_time, records):
    relevant = []
    for i in range(len(records)):
        if two_hours_close(records[i][4], wake_time, Util.min_in_2_hours):
            relevant.append(records[i])

    relevant = np.array(relevant)
    if len(relevant) == 0:
        time = wake_time - Util.min_in_8_hours
        if time < 0:
            time += Util.num_min_in_day
        return time
    return ((np.array(relevant[:, 3]).astype(float) * np.array(relevant[:, 2]).astype(float)).sum()) / (
        np.array(relevant[:, 2]).astype(float).sum())


# given 2 array of hours, check how much the second array can contribute in similarity to the first array
# this similarity is not symmetric
def hours_array_sim(hours1, hours2):
    div = len(hours1)
    sim_score = 0
    for k1, val1 in hours1.items():
        # limit how much one hour can contribute to the sim score
        limit = Util.per_hour_sim_factor / div
        sum_val = 0
        for k2, val2 in hours2.items():
            if two_hours_close(k1, k2, Util.min_in_1_hour):
                sum_val += (1 - (dist_between_hours(k1, k2) / Util.min_in_1_hour)) * (min((val2 / val1), 1))
        sim_score += min(1.0, sum_val / div, limit)
...
class Recommender:
    ...
    def content_sim_between_2users(self, a, b):
        ...
        if abs(a[0] - b[0]) < Util.days_in_5_years:
            similarity += (1 - (abs((a[0] - b[0]) / Util.days_in_5_years))) * Util.content_sim_weight
        # is same gender
        if a[1] == b[1]:
            similarity += Util.content_sim_weight
        # height is similar if at the very least they are 20 cem apart
        if abs(a[2] - b[2]) < Util.height_sim:
            similarity += (1 - (abs((a[2] - b[2]) / Util.height_sim))) * Util.content_sim_weight
        # weight is similar if at the very least they are 15 kilos apart
        if abs(a[3] - b[3]) < Util.weight_sim:
            similarity += (1 - (abs((a[3] - b[3]) / Util.weight_sim))) * Util.content_sim_weight
        return similarity
    ...
    def train(self, users, sleep_records):
        ...
        content_sim = self.calc_similarity(content_mat, self.content_sim_between_2users)
        ...
        self.similarity = content_sim * Util.content_met_weight + start_sim_mat * Util.hours_met_weight + end_sim_mat * Util.hours_met_weight
